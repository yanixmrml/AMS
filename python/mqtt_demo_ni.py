...
def on_message(client, userdata, msg):
	...
	if(id_num<100000):
		...
		cursor = cnx.cursor()
		cursor.execute("""SELECT f.faculty_id FROM faculty f WHERE f.faculty_id = %s""",(id_num))
		data = cursor.fetchone()
		
		if(data is not None):
			cursor = cnx.cursor()
			cursor.execute("""SELECT s.schedule_day FROM student_courses s WHERE s.faculty_id  %s AND %s BETWEEN s.time_start AND s.time_end""",(id_num,current_datetime))
			data = cursor.fetchone()
			if(data is not None):
				schedule_day = data[0]
		...
		if(data is not None):
			cursor = cnx.cursor()
			cursor.execute("""SELECT s.student_course_id, s.faculty_id, s.schedule_day, s.time_start, s.time_end FROM student_courses s WHERE s.faculty_id  %s AND %s BETWEEN s.time_start AND s.time_end""",(id_num,current_datetime))
			data = cursor.fetchone()
			cursor.close();
			if(data is not None):
				...
				if(schedule_day.find(days[current_datetime.weekday()])>=0):
					...
					cursor = cnx.cursor()
					cursor.execute("""SELECT a.faculty_id FROM attendance a WHERE a.faculty_id = % AND a.date_attended BETWEEM  %s AND %s""",(faculty_id,time_start,time_end))
					data = cursor.fetchone()
					cursor.close()
					if(data is not None):		
						cursor = cnx.cursor()
