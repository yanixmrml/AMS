...
def on_message(client, userdata, msg):
	...
	if(id_num<100000):
		...
		if(data is not None):
			faculty_id = data[0]					
			cursor = cnx.cursor()
			cursor.execute("""SELECT s.schedule_day, time_on, time_off FROM student_courses s WHERE s.faculty_id = %s AND %s BETWEEN s.time_start AND s.time_end""",(faculty_id,current_time.strftime("%H:%M")))
			data = cursor.fetchone()
				...
				if(data is not None):
					client.publish(topic_2, "Has logged in.")	
                                    ...
                                    if(schedule_day.find(days[current_datetime.weekday()])>=0):
                                        ...
                                        #Update student in attendance log
                                        cursor = cnx.cursor()
                                        #cursor.execute(("""UPDATE """),(faculty_id,1))
                                        cursor.close()
                                    else:
