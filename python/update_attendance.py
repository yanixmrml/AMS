        ...
        def on_message(client, userdata, msg):
                ...
                if(id_num<100000):
                        ...
                        if(data is not None):    
                                faculty_id = data[0]					
                                cursor = cnx.cursor()
                                cursor.execute("""SELECT s.schedule_day, time_start, time_end, s.section FROM student_courses s WHERE s.faculty_id = %s AND %s BETWEEN s.time_start AND s.time_end""",(faculty_id,current_time.strftime("%H:%M")))
                                data = cursor.fetchone()
                                ...
                                if(data is not None):
                                        ...
                                        cursor = cnx.cursor()
                                        cursor.execute("""SELECT a.faculty_id FROM attendance a WHERE a.faculty_id = %s AND TIME(a.date_attended) BETWEEN  TIME(%s) AND TIME(%s)""",(faculty_id,time_start,time_end))
                                        data = cursor.fetchone()
                                        cursor.close()
                                        ...
                                        if(data is not None):
                                                ...
                                                if(schedule_day.find(days[current_datetime.weekday()])>=0):
                                                        ...
                                                        cursor = cnx.cursor()
                                                        cursor.execute(("""UPDATE attendance a SET a.status = 1 WHERE a.student_course_id IN
                                                                        (SELECT sc.student_course_id FROM student_courses sc WHERE sc.faculty_id = %s AND sc.section = %s)
                                                                        AND TIME(a.date_attended) BETWEEN TIME(%s) AND TIME(%s)"""),(faculty_id,section,time_start,time_end))
                                                        cursor.close()
                                                else:
                                                        client.publish(topic_2, "Not your sched.")
                                                        print("Not your sched.");
                                else:
                                        client.publish(topic_2, "Not your sched.")
                                        print("Not your sched.");
                        else:
                                ...
                                if(data is not None):
                                        ...
                                        if(schedule_day.find(days[current_datetime.weekday()])>=0):
                                                ...
                                                if(data is not None):
                                                        ...
                                                        cursor = cnx.cursor()
                                                        cursor.execute("""SELECT a.faculty_id FROM attendance a WHERE a.faculty_id = %s AND TIME(a.date_attended) BETWEEN  %s AND %s""",(faculty_id,time_start,time_end))
                                                        data = cursor.fetchone()
                                                        cursor.close()
