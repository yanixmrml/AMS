...
def on_message(client, userdata, msg):
	...
	if(id_num<100000):
		...
		if(data is not None):
			student_id = data[0]
			cursor = cnx.cursor()
			cursor.execute("""SELECT s.student_course_id, s.faculty_id, s.schedule_day, s.time_start, s.time_end FROM student_courses s WHERE s.student_id = %s AND %s BETWEEN s.time_start AND s.time_end""",(student_id,current_time))
			data = cursor.fetchone()
			cursor.close();
			if(data is not None):
